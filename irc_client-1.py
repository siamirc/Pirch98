...
class PIRCHMainWindow(QMainWindow):
    ...
    def on_user_list(self, channel, users):
        """ ได้รับรายชื่อผู้ใช้ทั้งหมดก้อนแรกจากการ JOIN ห้องแชทสำเร็จ """
        chan_key = channel.lower()
        if chan_key in self.rooms:
            cleaned_users = []
            for u in users:
                u_str = u.strip()
                if u_str:
                    cleaned_users.append(u_str)
                    
            # บันทึกและส่งไปจัดหมวดหมู่ยศคลาสสิกขวามือ
            self.rooms[chan_key]["users"] = cleaned_users
            self.update_user_list_ui(channel, cleaned_users)
